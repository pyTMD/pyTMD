...
class forward(model):
    ...
    def initialize(self, **kwargs):
        ...
        self.dtype = np.array(
            [kwargs["u0"][0], kwargs["v0"][0], kwargs["z0"][0]]
        ).dtype
    ...
    def _linear_operators(self):
        ...
        DIVzu = (
            ...
            * BDx
            * self.d0(self.mask_z)
            * self.d0(IE * self.mask_z)
        )
        DIVzv = (
            self.d0(self.H0)
            * BDy
            * self.d0(self.mask_z)
            * self.d0(IN * self.mask_z)
        )

        # operators for the u nodes
        DY0 = (
            self.d0(self.mask_z) * self.d0(IN * self.mask_z) * FDy
            + self.d0(self.mask_z)
            * self.d0(1 - IN * self.mask_z)
            * (self.d0(1.0 / dy) * (-2 * I))
            + self.d0(1 - self.mask_z)
            * self.d0(IN * self.mask_z)
            * (self.d0(1.0 / dy) * (2 * IN))
        )
        ...
        DX0 = (
            self.d0(self.mask_z) * self.d0(IE * self.mask_z) * FDx
            + self.d0(self.mask_z)
            * self.d0(1 - IE * self.mask_z)
            * (self.d0(1.0 / dx) * (-2 * I))
            + self.d0(1 - self.mask_z)
            * self.d0(IE * self.mask_z)
            * (self.d0(1.0 / dx) * (2 * IE))
        ...
        self.M = scipy.sparse.vstack(
            [
                scipy.sparse.hstack(
                    [-self.nu * DEL2u, -self.d0(f_u) * Vu, self.d0(gp) * FDx]
                ),
                scipy.sparse.hstack(
                    [self.d0(f_v) * Uv, -self.nu * DEL2v, self.d0(gp) * FDy]
                ),
                scipy.sparse.hstack(
                    [DIVzu, DIVzv, scipy.sparse.csc_matrix((n, n))]
                ),
            ]
        ).tocsc()
    ...
    def _enforce_boundary(self):
        ...
        if self.iscomplex and self.has_ob:
            th = polynomial_sum(self._pc, self.t)
            self._u[self._iob] = self._u0.real * np.cos(
                th
            ) - self._u0.imag * np.sin(th)
            self._v[self._iob] = self._v0.real * np.cos(
                th
            ) - self._v0.imag * np.sin(th)
            self._z[self._iob] = self._z0.real * np.cos(
                th
