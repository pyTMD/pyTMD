...
- **Check 1:** Are the coordinates are over land or outside the model domain?
...
Post on the ``pyTMD`` `discussions board <https://github.com/pyTMD/pyTMD/discussions>`_ if you want help choosing a model.
...
Similar to the the case that returned ``NaN``, the returned values may be finite but mapped to an incorrect location.
