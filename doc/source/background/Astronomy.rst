...
``pyTMD`` can calculate these positions using either high-precision numerical :ref:`ephemerides from JPL <jpl-ephemerides>` or one of several analytical approximations.
The :ref:`JPL ephemerides <jpl-ephemerides>` are numerically integrated solutions to the `equations of motion for the planets, Sun and Moon <https://ssd.jpl.nasa.gov/planets/orbits.html>`_ and are provided in the form of binary "kernels".
