...
for i, line in enumerate(CTE):
    ...
    cons = pyTMD.constituents._to_constituent_id(
        arguments, arguments=6, raise_error=False
    )
...
for i, ax in enumerate(ax2):
    ...
    at = offsetbox.AnchoredText(
        labels[i], loc=2, pad=0, borderpad=0.5, frameon=False, prop=prop
    )
    ax.axes.add_artist(at)
    # set ticks for inset axes
    ax.get_xaxis().set_tick_params(
        which="both", direction="in", color=plot_colors[i]
    )
